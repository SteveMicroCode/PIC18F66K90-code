 ...
 * 
 * This is a development code version that introduces keyboard scanning using
 * the keyscan() function. A 2x8 matrix of keys can be read see the
 * TM1637 datasheet for more details. 
 * 
 * See PIC12F1840 for demo use of this library with a 4 digit display
 * Has been tested on a PIC18F6410 and PIC18F66K90 additionally
 ...
 * 
 * Note dio pin must be configured as a digital input, beware any default to analogue
 * 
...
#define BIT_TIME 10              // Bit time in us, note TM1637 max data rate of 500kHz assuming short signal path
                                 // Max clock frequency is reduced to 250 kHz if keyboard scanning enabled
...
const uint8_t tm1637ByteReadKeys = 0x42;       // 0x40 = data command, | with 0x02 to start a read
...
uint8_t tm1637MaxDigits = 0;         // Total no of digits for the display in use
...
/*********************************************************************************************
 tm1637ByteWrite(char bWrite) - function writes one byte
 NB dio pin should be configured for digital i/o, beware any default to analogue
*********************************************************************************************/
...
/***********************************************************************************************
 *  keyscan() - function to scan key switch input via TM1637, 8x2 switch matrix, K1/K2 1..8
 *  nb. dio pin must be configured for digital i/o, beware any PIC default to analogue
 *  Datasheet specs max clock of 250 kHz if reading keys, adjust BIT_TIME accordingly
 *  Read may be affected by pullups used though tested and seemed reliable with 10K and 10us bit time
 *  Lower the TM1637 pullups or extend bit time if needed
 *  Calling application responsible for timing of the keypad reads and calling code needs
 *  to deal with repeated sequentional/invalid inputs by user, log any keypress and follow
 *  with a timeout until processed
 ***********************************************************************************************/
...
uint8_t keyscan(void)
{
  ...
  tm1637StartCondition();                     // Set start 
  
  // Send key scan data request command:
  uint8_t success = tm1637ByteWrite(tm1637ByteReadKeys);   // ACK check done by ByteWrite(), on exit clk and dio set as ops,low
 
  // Set dio as input for returned key scan data:
  *portTris |= 1<<tm1637dioTrisBit;           // Set data tris, makes dio an input, will pullup high, receives data byte
  __delay_us(BIT_TIME);
  
  // Read in the key scan data byte:
  for (uint8_t ctr = 0; ctr <= 7; ctr++)
    {
      *portLatch &= ~(1<<tm1637clkTrisBit);      // Latch set low so clock will go low when clear tris bit
      *portTris &= ~(1<<tm1637clkTrisBit);       // Clear clk tris bit
      __delay_us(BIT_TIME);
      scanData >>= 1;
      scanData |= (*portPins & (1<<tm1637dioTrisBit) ? 0x80 : 0x00);     // Read port dio line, nb read pin not latch
      *portTris |= 1<<tm1637clkTrisBit;          // Set tris so clk goes high
      __delay_us(BIT_TIME);
    }
  
    // Set dio as output/low after the read:
    *portTris &= ~(1<<tm1637dioTrisBit);        // Clear data tris bit
    *portLatch &= ~(1<<tm1637dioTrisBit);       // Data low
    __delay_us(BIT_TIME);
    
    // Take clk low to generate ACK:
     *portTris &= ~(1<<tm1637clkTrisBit);       // Clear clk tris bit
     *portLatch &= ~(1<<tm1637clkTrisBit);      // Clock low;
     __delay_us(BIT_TIME);
     *portTris |= 1<<tm1637clkTrisBit;          // Set tris so clk goes high (pullup)
     __delay_us(BIT_TIME);
    
    tm1637StopCondition();                      // Generate stop condition
    
    // As clocked in here the scanData byte order is reversed cf the TM1637 datasheet values
    // No key pressed == 255, K1/S1 -> K2/S8 == 247 -> 232 decimal, note descending values
    // Code here converts this to: K1/S1 == 1, K1/S8 == 8, K2/S1 == 9, K2/S8 == 16, ascending
    // Return is 0..16, zero now indicates no keypress, 1..16 == keys in ascending order
    
    uint8_t keyNo = 0;
    if (scanData <255)                          // If return is 255 no key press detected, do not process
      {
        for (uint8_t ctr = 247; ctr > 231; ctr --)      // Process any return starting 247 == K1/S1
        { 
            keyNo ++ ;                          // Increment key number so K1/S1 is 1, K1/S2 2, thru K2/S8 == 16
            if (ctr == scanData)
                break;                          // Stop processing once keyNo is coded   
        }
      }
    
    return keyNo;                               // Returns the keys 1..16, 0 == no key
}